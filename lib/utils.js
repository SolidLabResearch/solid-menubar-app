...
function removeTodayAndBefore(data) {
  const today = new Date();

  for (const el of data) {
    const filteredDates = [];

    for (const d of el.dates) {
      if (new Date(d) > today) {
...
module.exports = {getDates, getAllDates, removeToday: removeTodayAndBefore, getVacationsToday}